...
**ยังไม่ได้ทำ:** **KDS ยังไม่ขึ้น "ซื้อกลับ · คิว 12"** (ครัวอ่านตั๋วแล้วไม่รู้ว่าของใคร
— ก้อนถัดไป ควรทำก่อนเปิดใช้จริง) · ฟอร์มกรอกชื่อลูกค้า · ไรเดอร์ทั้งหมด
· ยังไม่ได้วัด responsive/overflow ของสองหน้าใหม่ด้วยสคริปต์ headless
...
**ก้อนถัดไป = บทที่ 13 ที่เหลือ** — ปุ่มส่วนลด · หน้าอ่าน `AuditLog` · ตาราง
`StaffSession` (ยกเลิก session ทันที) · จัดการพนักงาน/รีเซ็ต PIN
ปลดล็อก spec §12 §13 §17 §20 §26 พร้อมกัน และเป็นเรื่องกันโกงซึ่งควรมีก่อนเปิดใช้จริง

- **ส่วนลด** — สูตรใน `calculateBill()` คอลัมน์ใน `Payment`/`Order` และบรรทัด
  บนใบเสร็จ **พร้อมหมดแล้ว** เหลือแค่ปุ่ม ซึ่งต้องมาพร้อมจำกัดตามตำแหน่ง +
  บังคับเหตุผล + `AuditLog` เสมอ
- **`AuditLog.ipAddress`** มีคอลัมน์อยู่แล้วแต่ยังไม่มีใครเขียนค่า (spec §20 ขอ)
- action ที่เขียน log แล้วตอนนี้: `payment.take` · `order_item.cancel` ·
  `table_session.abandon` · `staff.login_failed` · `menu.*` · **`receipt.print`**

**หลังจากนั้น:** หน้า Settings (ถูกที่สุด ข้อมูลพร้อมหมด — spec §22 §24) →
บทที่ 15 รายงาน/ปิดกะ → ย้าย/รวมโต๊ะที่ค้างจากบทที่ 9 → บทที่ 14 สต็อก → Refund
