...
- **i18n:** `/admin/menu` + `admin-nav` + `(admin)/layout` + `live-refresh` เป็นอังกฤษแล้ว
  ...
  ที่ยังเป็นไทยบนหน้านั้นคือ **ข้อมูลใน DB** (ชื่อเมนู/หมวด/สถานี) ไม่ใช่ UI

**⚠ งานค้าง: smoke สามชุดยัง FAIL รวม 11 เคส (takeaway 8 · staff-session 2 ·
staff-admin 1) — มีอยู่ก่อนก้อนนี้แล้ว** (ยืนยันด้วย `git stash` ได้เลขเดียวกัน)
เป็น assertion ที่ยังเทียบ **สตริงภาษาไทย** ที่ i18n ก้อนก่อนแปลไปแล้ว
ต้องไล่แก้ให้ตรง ไม่งั้น FAIL จริงในอนาคตจะถูกกลบ
...
- **⚠ ข้อมูลเมนูใน DB เป็นอังกฤษแล้วทั้งหมด** (ชื่อเมนู/หมวด/สถานี/ตัวเลือก)
  — ข้อความเดิมใน CLAUDE.md ที่ว่า "ที่ยังเป็นไทยคือข้อมูลใน DB" **ไม่จริงแล้ว**
  · ปลอดภัยกับประวัติเพราะทุกบิลเก็บ `nameSnapshot` ของตัวเอง ใบเสร็จเก่ายังเป็นไทย
...
### ⚠ กำลังทำค้างอยู่: สลับภาษา vi/en + เปลี่ยนสาขาเป็น VND (2026-09-09)

ผู้ใช้เปลี่ยนทิศทางกลางทาง — **บทที่ 15 ถูกเลื่อนออกไปก่อน** ก้อนที่กำลังทำคือ
"เปลี่ยนภาษาหลักของ POS เป็นเวียดนาม+อังกฤษ และใช้สกุลเงิน VND"

- สาขา `feat/i18n-vi-en-vnd` — **ทำไป 3 จาก 11 Task · ปลายสาขาเขียว**
  (`tsc` 0 · `eslint` สะอาด · `build` ผ่าน)
- งานค้างกลางทางของ Task 4 อยู่ที่สาขา `wip/i18n-task4` (`e3134d1`) — **build ไม่ผ่าน** โดยตั้งใจ
- **อ่านสามไฟล์นี้ก่อนทำต่อ:**
  `archive/report/2026-09-09-i18n-vnd-progress.md` (สถานะ + ก้าวถัดไป) ·
  `...-plan-i18n-vnd-design.md` (spec) · `...-plan-i18n-vnd-tasks.md` (แผน 11 Task)

**กฎใหม่ที่บังคับใช้แล้วและห้ามฝ่าฝืน:**

- **`lib/server/*` ห้ามแปลภาษา — คืน `errorKey` + `params` เท่านั้น** หน้าจอเป็นคนแปล
  ผลพลอยได้คือ smoke ตรวจ **คีย์** ไม่ใช่ประโยค ซึ่งเป็นรากของบั๊กที่ทำให้เทสต์
  11 เคสพังตอนแปลเป็นอังกฤษรอบก่อน
- **`lib/i18n/vi.ts` เป็นต้นฉบับของรายชื่อคีย์** · `en.ts` เป็น `Record<MessageKey, string>`
  → แปลตกหล่น = **build error** ไม่ใช่คีย์ดิบโผล่บนจอ (ยืนยันแล้วว่า `TS2741` ฟ้องจริง)
- **ห้ามใส่ locale ลง URL** — QR ที่พิมพ์ไปแล้วเข้ารหัส `/t/<tableCode>` ไว้ ภาษาอยู่ใน cookie
- **ฟอนต์:** Archivo ขอ subset `vietnamese` แล้ว (ตัด Noto Sans Thai ออก) —
  subset นั้นมี **U+20AB คือสัญลักษณ์ ₫** ด้วย ถ้าถอดออกสัญลักษณ์เงินจะเพี้ยนทุกจอ
- **สาขายังเป็น THB อยู่** — การเปลี่ยนเป็น VND (+ `Asia/Ho_Chi_Minh`, VAT 8%,
  เซอร์วิสชาร์จ 5%, `pricesIncludeVat: false`) เป็น Task 9 และ **ต้อง `npm run db:reset`**

**smoke ตอนนี้ 810 PASS · 11 FAIL** — ทั้งหมดเป็น assertion ที่ยังเทียบประโยค
ไม่ใช่พฤติกรรมผิด (ตรวจทีละเคสแล้ว) · Task 10 เป็นเจ้าของงานนี้

---

**ก้อนถัดไปหลังจบ i18n = บทที่ 15 รายงาน/ปิดกะ** (X report · ปิดกะนับเงินสดจริง · Z report)
